...
public sealed class SolAtmosphereController : MonoBehaviour
{
    ...
    /// <summary>
    /// Spatial filter strength the volumetric composite will actually apply. Exposed
    /// because the renderer feature needs to know whether recording the filter pass would
    /// change anything: below High the strength is zero, the shader early-returns the
    /// centre tap, and the pass degenerates into a half-resolution copy. Per-camera
    /// overrides deliberately do not rewrite the upsample globals, so this controller
    /// value -- not the camera-adjusted quality -- is what the shader reads.
    /// </summary>
    public float CurrentSpatialFilterStrength => SettingsSpatialFilterStrength;
