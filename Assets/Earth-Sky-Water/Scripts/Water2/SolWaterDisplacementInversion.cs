...
namespace Sol.Water
{
    ...
    public static class SolWaterDisplacementInversion
    {
        ...
        /// <summary>
        /// Evaluates horizontal displacement at a sample position.
        ///
        /// An interface on a constrained struct rather than a delegate. The solve runs up
        /// to nine evaluations per query and every query is on a hot path -- buoyancy
        /// calls it once per sample point per physics step -- so a capturing lambda here
        /// allocated a closure and a delegate on every call, in the service of a component
        /// documented as allocation-free. With `where T : struct`, the JIT specializes the
        /// generic and the interface call devirtualizes, so this costs nothing.
        /// </summary>
        public interface ISolWaterHorizontalDisplacement
        {
            Vector2 Evaluate(Vector2 samplePositionXZ);
        }
        ...
        /// </summary>
        public static Vector2 Solve<TDisplacement>(Vector2 targetXZ,
            in TDisplacement displacement, int iterations = DefaultIterations)
            where TDisplacement : struct, ISolWaterHorizontalDisplacement
        {
            Vector2 estimate = targetXZ;
            for (int i = 0; i < Mathf.Max(0, iterations); i++)
            {
                Vector2 displaced = estimate + displacement.Evaluate(estimate);
        ...
        /// </summary>
        public static float ResidualError<TDisplacement>(Vector2 targetXZ, Vector2 solved,
            in TDisplacement displacement)
            where TDisplacement : struct, ISolWaterHorizontalDisplacement
        {
            Vector2 displaced = solved + displacement.Evaluate(solved);
