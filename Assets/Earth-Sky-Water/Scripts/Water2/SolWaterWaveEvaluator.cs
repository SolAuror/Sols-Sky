...
namespace Sol.Water
{
    ...
    public static class SolWaterWaveEvaluator
    {
            ...
            out float foam)
        {
            ...
            float sampleRadius = ResolveShorelineSampleRadius(profile);
        ...
        /// <summary>
        /// One texel of the shoreline source, in metres, clamped to the same range
        /// <c>SolWaterShorelineGradient</c> uses. The gradient the breakers turn toward is
        /// a finite difference, so this has to come from whichever source is actually
        /// being sampled -- reading the profile's texture dimensions while the live field
        /// is in use would measure the gradient over the wrong step, and dereference a
        /// texture that a terrain-driven scene has no reason to assign at all.
        /// </summary>
        static float ResolveShorelineSampleRadius(SolWaterProfile profile)
        {
            SolShorelineField field = SolTerrainShoreline.OceanField;
            if (field != null)
            {
                return Mathf.Clamp(Mathf.Max(
                    field.Mapping.z / Mathf.Max(1, field.Width),
                    field.Mapping.w / Mathf.Max(1, field.Height)), 0.25f, 4f);
            }
        ...
        /// <summary>
        /// The CPU mirror of <c>SolSampleShorelineData</c>.
        ///
        /// The live field is preferred over the profile's baked texture for the same
        /// reason the shader prefers it, and it is cheaper here as well: it samples float
        /// arrays directly rather than going through <c>GetPixelBilinear</c>, which is a
        /// managed call per tap and needs the texture to have been imported readable.
        /// A breaker query takes five of these taps, on every buoyancy sample point of
        /// every floating body.
        ///
        /// Only the infinite path reaches here -- <c>EvaluateFinite</c> passes
        /// evaluateShoreline false -- so this is already ocean-only and needs no test of
        /// its own for which body the field belongs to.
        /// </summary>
            ...
            out float depth, out float shoreDistance)
        {
            ...
            SolShorelineField field = SolTerrainShoreline.OceanField;
            if (field != null)
                return field.TrySample(logicalXZ, out depth, out shoreDistance);
