...
namespace Sol.Water
{
    ...
    {
        [Header("Wetness Sources")]
        [Tooltip("How strongly rain alone wets the terrain. Driven by the environment's "
            + "smoothed wetness accumulator, so ground dries out gradually after a shower.")]
        [Range(0f, 1f)] public float rainWetness = 1f;

        [Tooltip("How strongly proximity to the water surface wets the terrain. This is "
            + "what produces the damp band along a shoreline.")]
        [Range(0f, 1f)] public float waterWetness = 1f;

        [Tooltip("Height above the water level, in metres, over which shoreline wetness "
            + "fades out.")]
        [Min(0.01f)] public float waterWetnessRange = 0.75f;

        [Tooltip("Distance inland, in metres, over which shoreline wetness fades out. "
            + "This is wave run-up across the ground, and it is what stops a flat beach "
            + "reading as wet for its whole width just because it sits low. Needs the "
            + "live shoreline field; without one, only the height range above applies.")]
        [Min(0.01f)] public float waterWetnessInlandRange = 6f;

        [Header("Wet Surface Response")]
        [Tooltip("Maximum albedo darkening applied to wet sand only. Other terrain "
            + "layers only gain smoothness.")]
        [Range(0f, 0.4f)] public float sandWetDarkening = 0.14f;

        [Tooltip("Target smoothness for fully wet terrain. Authored values above this "
            + "are preserved, so already-glossy surfaces are never dulled.")]
        [Range(0f, 0.8f)] public float wetSmoothness = 0.48f;

        [Header("Sand Layer")]
        [Tooltip("Terrain whose sand layer receives wet darkening. Empty uses the active "
            + "terrain.")]
        public Terrain wetnessTerrain;

        [Tooltip("Sand TerrainLayer to darken when wet. Empty resolves a layer named "
            + "TerrainLayer_Sand.")]
        ...
        static readonly int TerrainShorelineDataId = Shader.PropertyToID("_Sol_TerrainShorelineData");
        static readonly int TerrainShorelineMappingId = Shader.PropertyToID("_Sol_TerrainShorelineMapping");
        static readonly int TerrainShorelineParamsId = Shader.PropertyToID("_Sol_TerrainShorelineParams");
        ...
        Vector4 _lastShorelineMapping = new(float.NaN, float.NaN, float.NaN, float.NaN);
        Vector4 _lastShorelineParams = new(float.NaN, float.NaN, float.NaN, float.NaN);
        Texture _lastSandMask;
        Texture _lastShorelineData;
        ...
        public void Invalidate()
        {
            ...
            _lastShorelineMapping = new Vector4(float.NaN, float.NaN, float.NaN, float.NaN);
            _lastShorelineParams = new Vector4(float.NaN, float.NaN, float.NaN, float.NaN);
            _lastSandMask = null;
            _lastShorelineData = null;
            _sandMaskDirty = true;
        }
        ...
        /// <summary>
        /// Republishes the ocean's shoreline field for the terrain shader.
        ///
        /// The terrain gets its own copy of the mapping rather than reading the water's,
        /// because the two are in different spaces: the water samples the field in logical
        /// coordinates and has <c>_SolWaterWorldOrigin</c> to convert with, while the
        /// terrain only ever has render-space positions. Subtracting the origin here is
        /// cheaper and less error-prone than giving every terrain shader a second uniform
        /// and the discipline to apply it.
        ///
        /// This is the same authority that already publishes the water level the height
        /// band is measured against, so the two halves of the shoreline damp band cannot
        /// come from different frames.
        /// </summary>
        void PushShorelineField()
        {
            SolShorelineField field = SolTerrainShoreline.OceanField;
            Texture data = field != null ? field.Texture : Texture2D.blackTexture;
            Vector4 mapping = Vector4.zero;
            Vector4 parameters = Vector4.zero;
            if (field != null)
            {
                SolDouble3 origin = SolWorldOriginService.Active?.LogicalOrigin ?? default;
                Vector4 logical = field.Mapping;
                mapping = new Vector4(
                    logical.x - (float)origin.X,
                    logical.y - (float)origin.Z,
                    logical.z,
                    logical.w);
                parameters = new Vector4(
                    field.DistanceRange,
                    Mathf.Max(0.01f, waterWetnessInlandRange),
                    0f,
                    1f);
            }

            if (_lastShorelineData != data)
            {
                Shader.SetGlobalTexture(TerrainShorelineDataId, data);
                _lastShorelineData = data;
            }
            if (_lastShorelineMapping != mapping)
            {
                Shader.SetGlobalVector(TerrainShorelineMappingId, mapping);
                _lastShorelineMapping = mapping;
            }
            if (_lastShorelineParams != parameters)
            {
                Shader.SetGlobalVector(TerrainShorelineParamsId, parameters);
                _lastShorelineParams = parameters;
            }
        }
