...
namespace Sol.Water
{
    ...
    public sealed class SolWaterWorld : MonoBehaviour
    {
        ...
        [Header("Authoring")]
        [Tooltip("Profile used by any water body that does not carry its own.")]
        [SerializeField] SolWaterProfile defaultProfile;
        [Tooltip("Scene-wide quality and feature policy. A body may disable a feature "
            + "but cannot exceed what this allows. Empty uses built-in defaults.")]
        [SerializeField] SolWaterQualityProfile qualityProfile;

        [Header("Resolved Dependencies")]
        [Tooltip("Source of the wave clock, wind and weather. Resolved from the active "
            + "environment world when empty.")]
        [SerializeField] SolEnvironmentWorld environmentWorld;
        [Tooltip("Gameplay query front-end. Added to this object automatically if absent.")]
        [SerializeField] SolWaterQueryService queryService;
        [Tooltip("Planar reflection capture. Added automatically; does nothing unless "
            + "planar reflections are enabled on both the quality profile and a body.")]
        [SerializeField] SolPlanarReflectionRenderer planarReflectionRenderer;
        [Tooltip("Terrain wetness authority. Added automatically so converted scenes "
            + "regain shoreline wetness without a second manual step.")]
        [SerializeField] SolWaterWetness wetness;
        [Tooltip("Builds the ocean's shoreline data from the live terrain. Added "
            + "automatically; does nothing in a scene with no terrain, where the profile's "
            + "baked shoreline texture is used instead.")]
        [SerializeField] SolTerrainShoreline terrainShoreline;
        ...
        public bool TrySampleApproximate(Vector3 localPosition, out SolWaterSurfaceSample sample)
        {
            ...
            {
                BodyHorizontalDisplacement displacement = new(
                    body, geometry.Position.y, WaveTime, environment);
        ...
        /// <summary>
        /// Horizontal wave displacement at a probe position, for the inversion solve.
        ///
        /// A readonly struct rather than a lambda: the solve evaluates this up to nine
        /// times per query, and a closure here allocated on every query -- including every
        /// buoyancy sample point of every floating rigidbody, every physics step.
        /// Readonly so passing it as `in` cannot make a defensive copy per call.
        /// </summary>
        readonly struct BodyHorizontalDisplacement
            : SolWaterDisplacementInversion.ISolWaterHorizontalDisplacement
        {
            readonly SolWaterBody _body;
            readonly float _surfaceY;
            readonly double _waveTime;
            readonly SolEnvironmentState _environment;

            internal BodyHorizontalDisplacement(SolWaterBody body, float surfaceY,
                double waveTime, in SolEnvironmentState environment)
            {
                _body = body;
                _surfaceY = surfaceY;
                _waveTime = waveTime;
            ...
            public Vector2 Evaluate(Vector2 samplePositionXZ)
            {
                Vector3 probe = new(samplePositionXZ.x, _surfaceY, samplePositionXZ.y);
                Vector3 offset = _body.EvaluateWaves(probe, _waveTime, _environment).Displacement;
        ...
        void ResolveDependencies()
        {
            ...
            // Same reasoning: the shoreline field is the only producer of shoreline data
            // in the project, so a scene that has to have it added by hand is a scene
            // whose waterline silently does not track its terrain.
            if (terrainShoreline == null)
                terrainShoreline = GetComponent<SolTerrainShoreline>();
            if (terrainShoreline == null)
                terrainShoreline = gameObject.AddComponent<SolTerrainShoreline>();
        }
