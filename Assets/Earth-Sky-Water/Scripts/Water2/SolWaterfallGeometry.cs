...
namespace Sol.Water
{
    ...
    public sealed class SolWaterfallGeometry : MonoBehaviour, ISolWaterGeometry
    {
        ...
        [Tooltip("Spline the water falls along, from lip to plunge point. Resolved from "
            + "this object when empty; it needs at least two knots.")]
        [SerializeField] SplineContainer splineContainer;
        [Tooltip("Longest step down the fall, in metres. Smaller values follow an "
            + "overhanging or curving drop more closely.")]
        [SerializeField, Min(0.1f)] float maximumSegmentLength = 0.75f;

        [Header("Sheet")]
        [Tooltip("Total width of the fall at the lip, in metres.")]
        [SerializeField, Min(0.1f)] float width = 6f;
        [Tooltip("Width multiplier down the fall, 0 at the lip to 1 at the plunge point. "
            + "Narrowing slightly reads as the sheet gathering as it accelerates.")]
        [SerializeField] AnimationCurve widthAlongFall = AnimationCurve.Linear(0f, 1f, 1f, 0.8f);
        [Tooltip("Vertical strands the sheet is split into. One reads as a smooth "
            + "curtain, several as separate falls.")]
        [SerializeField, Range(1, MaximumRibbons)] int ribbonCount = 5;
        [Tooltip("Gap between ribbons, as a fraction of the width. Zero leaves an "
            + "unbroken sheet.")]
        [SerializeField, Range(0f, 1f)] float ribbonGap = 0.08f;
        [Tooltip("Deterministic per-ribbon width and offset variation, so the strands do "
            + "not read as a regular comb. Geometry-local, so it cannot shift with the "
            + "camera or a floating-origin shift.")]
        [SerializeField, Min(0f)] float ribbonBreakup = 0.12f;
        [Tooltip("Depth of the sheet, in metres. Gives the fall volume when seen edge-on.")]
        [SerializeField, Min(0.01f)] float sheetThickness = 0.2f;
        [Tooltip("Metres of fall covered by one UV tile. Sets the scale of every surface "
            + "texture on this body.")]
        [SerializeField, Min(0.01f)] float uvMetersPerTile = 2f;

        [Header("Flow and Foam")]
        [Tooltip("Fall speed in metres per second. Drives foam advection and the flow "
            + "reported to gameplay.")]
        [SerializeField, Min(0f)] float flowSpeed = 8f;
        [Tooltip("Foam at the lip, where the water first goes over.")]
        [SerializeField, Range(0f, 1f)] float lipFoam = 0.2f;
        [Tooltip("Foam where the fall lands. Usually the whitest part of the effect.")]
        [SerializeField, Range(0f, 1f)] float impactFoam = 1f;
        [Tooltip("Fraction of the fall's length, measured from the bottom, treated as "
            + "the impact zone that carries impact foam.")]
        [SerializeField, Range(0.05f, 1f)] float impactZoneRadius = 0.35f;

        [Header("Plunge Pool")]
        [Tooltip("Generate a disc of still water where the fall lands.")]
        [SerializeField] bool generatePlungePool = true;
        [Tooltip("Radius of the plunge pool, in metres.")]
        [SerializeField, Min(0.1f)] float plungePoolRadius = 4f;
        [Tooltip("Pool depth in metres. Reported to buoyancy and used to size the "
            + "culling bounds; it does not generate a bed.")]
        [SerializeField, Min(0.05f)] float plungePoolDepth = 2f;
        [Tooltip("Segments around the pool rim. Raise it only if the circle reads as a "
            + "polygon at close range.")]
        ...
        [Tooltip("Regenerate the mesh whenever the spline is edited. Turn off on very "
            + "dense falls and rebuild manually from the component context menu.")]
