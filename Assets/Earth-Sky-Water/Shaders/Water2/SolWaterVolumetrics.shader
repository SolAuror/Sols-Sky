    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 VolumetricFragment(Varyings input) : SV_Target
            {
                ...
                // Steps actually walked, which is not `steps` whenever the transmittance
                // break below fires. Dividing the shadow sum by the full count instead
                // reported a march that ended early as if its remaining steps had been
                // fully lit, biasing alpha low -- and the surface pass darkens its own
                // scattering by up to 0.35x through lerp(0.35, 1, volume.a), so turbid
                // water came out darker the sooner the march gave up on it.
                int stepsTaken = 0;
                // Loop-invariant: stepSize and turbidity are both fixed for this pixel.
                ...
                for (int i = 0; i < 32; i++)
                {
                    ...
                    //
                    // The caustic term is deliberately signed -- the dark cells between
                    // the filaments are most of what makes the pattern legible -- but a
                    // caustic field reaches -1.15, so (1 + caustic) could go negative and
                    // SUBTRACT light from the shaft. Caustics redistribute light within
                    // the volume; they cannot take a slice below unlit.
                    scattering += transmittance * (1.0 - sliceTransmittance)
                        * shadow * max(0.0, 1.0 + caustic);
                ...
                // its own scattering under the same occluders. Averaged over the steps
                // that were actually walked, not the steps that were planned.
                return half4(inscatter, saturate(shadowAccumulation / max(1, stepsTaken)));
            }
            ENDHLSL
        }
