Shader "Sol/Skybox"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // 3-octave noise for star field.
            float StarNoise(float2 uv, float scale)
            {
                float result = 0.0;
                [unroll]
                for (int i = 0; i < 3; i++)
            ...
            float4 Frag(Varyings IN) : SV_Target
            {
                ...
                float3 sky = saturate(_ZenithColor.rgb  * zenithMask
                                    + _HorizonColor.rgb * horizonMask
                                    + _NadirColor.rgb   * nadirMask);

                // ── Stars ─────────────────────────────
                float2 starUV = float2(dir.x, dir.z) / max(y, 1e-4);
                float  noise  = StarNoise(starUV, _StarHeight);
                float  rnd    = frac(sin(dot(float2(noise, noise), float2(12.9898, 78.233))) * 43758.5453);
                float3 stars  = SampleStarGradient(rnd)
                              * _StarIntensity
                              * pow(max(noise, 0.0), _StarPower)
                              * zenithMask;
                ...
                float moonOverSun = DiscMask(moonDot, _SunDiscSize * 1.001);
                float occluder    = moonOverSun * _SolarEclipseFactor;

                // Corona ring: visible at the edge of the occluder during eclipse.
                // Broader than the sun disc, dims quickly away from the edge.
                float coronaRing = saturate(sunMask * 2.0 - moonOverSun)
                                 * _SolarEclipseFactor;
                ...
                float density = CloudFBM(warped * _CloudScale);
                density  = smoothstep(_CloudCoverage, _CloudCoverage + 0.2, density);
                density *= _CloudDensity * saturate(dir.y * 10.0);

                float3 cloudCol = lerp(_CloudShadowColor.rgb, _CloudColor.rgb,
                                       saturate(sunDot * 0.5 + 0.6));

                // ── Atmosphere ─────────────────────────
                float haze = pow(1.0 - abs(y), 4.0) * _HazeIntensity;

                // ── Composite ─────────────────────────
                // Layer order: sky → sun/moon (additive) → stars (behind clouds & bodies)
                //            → clouds (over everything) → atmosphere (additive on top)
                ...
                float3 color = sky;
                color += stars * (1.0 - bodyMask);   // stars behind bodies
                color += bodies;                      // sun & moon on top of sky
                color  = lerp(color, cloudCol, density); // clouds over everything
                color += _HorizonColor.rgb * (glow + haze); // atmospheric scatter
