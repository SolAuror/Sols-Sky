...
{
    // Slightly different bases prevent the four periodic FFT domains from
    // resolving into one shared world-axis lattice without destroying the
    // authored wind direction at any individual scale.
    float2 cs = cascade == 0 ? float2(1.0, 0.0)
...
void SolEvaluateWaterPixelNormalFoam(float2 localXZ, out float3 normal, out float foam)
{
    normal = float3(0, 1, 0);
    foam = 0;
    if (_SolWaterSpectralParams.x < 0.5 || _SolWaterSpectralParams.y <= 0.0001)
    ...
    for (int cascade = 0; cascade < min((int)_SolWaterSpectralParams.x, 4); cascade++)
    {
        ...
        float4 centerSample = SAMPLE_TEXTURE2D_ARRAY(
            _SolWaterSpectralNormalFoam, sampler_SolWaterSpectralNormalFoam,
            spectralUV, cascade);
        float tapExtent = min(0.75, footprintTexels * 0.25);
        float4 positiveSample = SAMPLE_TEXTURE2D_ARRAY(
            _SolWaterSpectralNormalFoam, sampler_SolWaterSpectralNormalFoam,
            frac(spectralUV + majorAxis * tapExtent), cascade);
        float4 negativeSample = SAMPLE_TEXTURE2D_ARRAY(
            _SolWaterSpectralNormalFoam, sampler_SolWaterSpectralNormalFoam,
            frac(spectralUV - majorAxis * tapExtent), cascade);
        float4 normalFoam = centerSample * 0.5
    ...
    foam = saturate(foam * _SolWaterFoamParams.x);
    float4 shorelineData = SolSampleShorelineData(localXZ);
    float shallowAttenuation = SolShallowWaveAttenuation(shorelineData);
