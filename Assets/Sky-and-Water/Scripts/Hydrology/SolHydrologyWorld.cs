...
namespace Sol.Hydrology
{
    ...
    public sealed class SolHydrologyWorld : MonoBehaviour
    {
        ...
        // Volumes and snow for nodes belonging to a cell that is currently unloaded.
        // Node state is world-scoped, so an unload/reload round trip already preserves
        // it; this exists so the seed survives a world re-initialize and so a reloaded
        // cell restores from where it actually was rather than from the asset's
        // authored initialVolume, which would silently create or destroy water.
        readonly Dictionary<string, double[]> _cellSeedVolumes = new();
        readonly Dictionary<string, double[]> _cellSeedSnow = new();
        ...
        void OnDisable() => UnsubscribeCellProvider();

        void SubscribeCellProvider()
        {
            if (_cellProvider == null || _subscribedProvider == _cellProvider)
                return;
            UnsubscribeCellProvider();
            _cellProvider.CellLoaded += OnCellLoaded;
            _cellProvider.CellUnloaded += OnCellUnloaded;
            _subscribedProvider = _cellProvider;
        }

        void UnsubscribeCellProvider()
        {
            if (_subscribedProvider == null)
                return;
            _subscribedProvider.CellLoaded -= OnCellLoaded;
            _subscribedProvider.CellUnloaded -= OnCellUnloaded;
            _subscribedProvider = null;
        }

        IEnvironmentCellProvider _subscribedProvider;

        void OnCellLoaded(SolEnvironmentCellId cell) => RestoreCellSeed(cell);

        void OnCellUnloaded(SolEnvironmentCellId cell) => CaptureCellSeed(cell);

        /// <summary>
        /// Records the current state of every node in a cell so it can be restored when
        /// the cell comes back. Total stored water is unchanged, so an unload is
        /// mass-neutral rather than a leak.
        /// </summary>
        public bool CaptureCellSeed(SolEnvironmentCellId cell)
        {
            if (asset == null || _volumes.Length == 0)
                return false;
            string key = cell.ToString();
            int count = 0;
            double[] volumes = new double[asset.nodes.Length];
            double[] snow = new double[asset.nodes.Length];
            for (int i = 0; i < asset.nodes.Length; i++)
            {
                if (asset.nodes[i].cellId != cell)
                    continue;
            ...
            if (count == 0)
                return false;
            _cellSeedVolumes[key] = volumes;
            _cellSeedSnow[key] = snow;
            return true;
        }

        /// <summary>
        /// Restores a previously captured cell seed. Returns false when the cell has
        /// never been captured, in which case its nodes keep whatever the asset seeded
        /// them with, which is the correct behaviour for a first load.
        /// </summary>
        public bool RestoreCellSeed(SolEnvironmentCellId cell)
        {
            if (asset == null || _volumes.Length == 0)
                return false;
            string key = cell.ToString();
            if (!_cellSeedVolumes.TryGetValue(key, out double[] volumes)
                || !_cellSeedSnow.TryGetValue(key, out double[] snow))
                return false;
            for (int i = 0; i < asset.nodes.Length; i++)
            {
                if (asset.nodes[i].cellId != cell)
                ...
                _volumes[i] = Math.Clamp(volumes[i], node.minimumVolume, node.maximumVolume);
                _snow[i] = Math.Max(0d, snow[i]);
                UpdateLevel(i);
                BindBodyLevel(i);
            }
            return true;
        ...
        /// <summary>Total stored water across every node, for mass conservation checks.</summary>
