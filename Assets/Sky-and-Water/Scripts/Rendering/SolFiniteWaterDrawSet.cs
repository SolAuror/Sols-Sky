...
namespace Sol.Water.Rendering
{
    ...
    internal sealed class SolFiniteWaterDrawSet
    {
            ...
            SolWaterDebugMode debugMode)
        {
            ...
            Vector3 representativeFlow = geometry != null ? geometry.RepresentativeFlow : body.AuthoredFlow;
            ...
            properties.SetVector(SolWaterShaderIds.SunParams,
                new Vector4(profile.sunSpecularStrength, profile.sunSpecularRoughness,
                    profile.sunSpecularMaximum, 0f));
            properties.SetVector(SolWaterShaderIds.RefractionParams,
            ...
            properties.SetVector(SolWaterShaderIds.VisibilityParams,
            ...
            properties.SetVector(SolWaterShaderIds.ShorelineBreakerParams,
            ...
            properties.SetVector(SolWaterShaderIds.ShorelineBreakerDetail,
            ...
            properties.SetTexture(SolWaterShaderIds.CausticTexture,
            ...
            // Debug views were dead on rivers, lakes, waterfalls and flood extents
            // because this path hardcoded the mode to zero while the ocean passed the
            // real one, so selecting a debug view silently showed final colour.
            properties.SetVector(SolWaterShaderIds.ReflectionParams,
                new Vector4(profile.skyReflectionStrength, (float)debugMode,
                    world.QualityProfile == null
                        || (world.QualityProfile.screenSpaceReflections
                            && world.QualityProfile.ssrResolutionScale > 0f) ? 1f : 0f,
