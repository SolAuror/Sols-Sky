Shader "Hidden/Sol/Water2/Resolve"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            // x: smear strength in normalized viewport units, y: distance falloff,
            // z: unused, w: enabled
            float4 _SolWaterAnisoParams;
            // The already temporally resolved reflection, not the raw trace.
            ...
            half4 AnisotropicFragment(Varyings input) : SV_Target
            {
                ...
                float distanceFade = saturate(1.0 - waterData.y * _SolWaterAnisoParams.y);
                ...
                for (int tap = 1; tap <= 4; tap++)
                {
                    ...
                    {
                        accumulated += SAMPLE_TEXTURE2D_X_LOD(_SolWaterSSRRawTexture,
